...
def create_model(
    ...
    freeze_encoder: bool = False,  # New argument to freeze encoder
    ...
    **kwargs,
    ...
    # Decide which layers to freeze based on the num_freeze_layers parameter
    if num_freeze_layers is not None:
        if num_freeze_layers < 0 or num_freeze_layers > len(list(model.encoder.children())):
            raise ValueError("Invalid value for num_freeze_layers.")
